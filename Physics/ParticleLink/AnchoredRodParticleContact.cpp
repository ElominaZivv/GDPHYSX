...
namespace physics
{
	void AnchoredRodParticleContact::Update() {}

	void AnchoredRodParticleContact::Resolve(float time)
	{
		ResolveVelocity(time);
		ResolveInterpenetration(time);
	}

	void AnchoredRodParticleContact::ResolveVelocity(float time)
	{
	...
	void AnchoredRodParticleContact::ResolveInterpenetration(float time)
	{
		//Compute Depth
		//GetDepth();
		...
		if (particles[1])
		{
			...
			//Translate a by the amount of -b
			particles[0]->pos -= P_b;
		}
